...
//                          [2] center                  shadowed
//
// CTRL is split on a principle rather than for convenience: a bit that shapes
// the waveform is double-buffered, a bit that switches the output OFF is not.
// Shadowing exists to stop a data change corrupting a pulse in flight, and
// delaying a stop by up to a period is exactly the wrong trade. So en and
// force_off land immediately; the counting-mode bit waits for the boundary,
// because changing mode mid-period would emit one malformed period.
//
...
//   center   = 0    -> edge-aligned, the simpler mode
...
module pwm_regs #(
    ...
    output reg              center_q,   // shadowed: shapes the waveform
    output reg              en_q,       // immediate: stops the waveform
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (addr)
                ...
                A_CTRL:   begin
                              en_q        <= wdata[0];   // immediate
                              force_off_q <= wdata[1];   // immediate
                              center_next <= wdata[2];   // staged
                          end
            endcase
        end
    end
