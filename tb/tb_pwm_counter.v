...
module tb_pwm_counter;
    ...
    pwm_counter #(.CNT_W(CNT_W)) dut (
        .clk (clk), .rst_n (rst_n), .en (en), .center (center),
        .period (period), .duty (duty),
        .cnt (cnt), .up (up), .update (update), .pwm_raw (pwm_raw)
    );
    ...
    integer exp_period = 1, exp_duty = 0, exp_top = 1;
    ...
    always @(negedge clk) begin
        ...
        if (en === 1'b1 && check_en === 1'b1) begin
            if (cnt >= exp_top)
                fail_num("P3 cnt out of range", cnt, exp_top - 1);
            ...
            if (upd_d === 1'b1) begin
                ...
                // Center-aligned visits every count twice, so both the period
                // and the high time double and the duty RATIO is unchanged.
                exp_top    = (period < 16'd1) ? 1 : period;
                exp_duty   = (duty >= exp_top) ? exp_top : duty;
                exp_period = center ? 2*exp_top : exp_top;
                exp_duty   = center ? 2*exp_duty : exp_duty;
            end
        end else begin
    ...
    initial begin
        ...
        exp_period = 10; exp_duty = 4; exp_top = 10;
        step(4);
        ...
        //--------------------------------------------------------------
        // Center-aligned. Same duty ratio, twice the clocks per period,
        // and cnt visits every value exactly twice per period.
        //--------------------------------------------------------------
        banner("center-aligned: period doubles, duty ratio does not");
        check_en = 1'b0; en = 1'b0; step(2);
        center = 1'b1; en = 1'b1; step(2); check_en = 1'b1;
        cfg(16'd10, 16'd4);  run_periods(10);   // expect 20 cycles, 8 high
        cfg(16'd10, 16'd0);  run_periods(6);    // 0%
        cfg(16'd10, 16'd10); run_periods(6);    // 100% -- reachable only
        cfg(16'd10, 16'd15); run_periods(6);    // because both turning
        cfg(16'd10, 16'd1);  run_periods(6);    // points are held
        cfg(16'd10, 16'd9);  run_periods(6);
        cfg(16'd3,  16'd1);  run_periods(6);
        cfg(16'd2,  16'd1);  run_periods(6);
        cfg(16'd1,  16'd1);  run_periods(6);
        cfg(16'd0,  16'd0);  run_periods(6);
        cfg(16'd5000, 16'd1250); run_periods(2);
        expect_checked;
        $display("       ok  100 MHz / (2*5000) = %0d Hz at 25%% duty",
                 100000000 / (2*5000));

        banner("center-aligned: randomized configs");
        for (i = 0; i < n_rand; i = i + 1) begin
            lfsr = {lfsr[14:0], lfsr[15]^lfsr[13]^lfsr[12]^lfsr[10]};
            cfg({10'd0, lfsr[5:0]} + 16'd2, {10'd0, lfsr[13:8]});
            step({28'd0, lfsr[3:0]});
            run_periods(3);
        end
        expect_checked;

        banner("switching back to edge-aligned");
        check_en = 1'b0; en = 1'b0; step(2);
        center = 1'b0; en = 1'b1; step(2); check_en = 1'b1;
        cfg(16'd10, 16'd4); run_periods(8);
        expect_checked;
